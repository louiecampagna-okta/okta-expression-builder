...
- **Group criteria completion** — inside `user.getGroups({'` or `user.isMemberOf({'`, autocomplete offers the four documented criteria keys plus `operator`. Pick one and it writes `': '` and opens the value position, where the suggestions come from **your tenant's actual groups**: real names for `group.profile.name`, real ids for `group.id` (each hinted with its group name), real source ids for `group.source.id`, and `OKTA_GROUP` / `APP_GROUP` / `BUILT_IN` for `group.type`. Inside a `.![ … ]` projection it offers the seven group fields Okta documents, then any others the fetched records carry.
