...
- **7 new templates** covering `EXACT` vs the default `STARTS_WITH`, OR-within-a-key, two criteria objects ANDed, and group projections as OIDC claims in both name spellings.
...
- **Clicking a Reference-tab example left the editor looking empty.** The handler set `ta.value` and scheduled an eval but never re-rendered the highlight layer. The visible text is the `<pre>` behind the textarea — the textarea's own glyphs are transparent — so the new expression evaluated correctly and produced correct results while showing nothing, or showing whatever was there before. Templates were unaffected because their handler did re-render, which is why the two behaved differently. Whole-value writes now go through one `setExpression` helper, and the audit greps the source to assert no `ta.value` write skips the re-render, since this invariant has now been the bug twice.
- **`.![name]` returned a list of nulls.** The classic reference documents this form explicitly — "the `user.getGroups` function supports the `.![name]` collection projection for the legacy configuration of group claims" — and works it as `user.getGroups({"group.profile.name": "Everyone", "operator": "STARTS_WITH"}).![name]` returning a list of group names. A group record carries the name at `profile.name`, so bare `name` resolved to nothing and every element came back null. Both spellings now work: `.![name]` from the classic reference and `.![profile.name]` from the Identity Engine one. The alias is scoped to the projection context, which is the scope the docs give it, rather than flattening the record everywhere. `description` is deliberately *not* aliased — only `name` is documented bare, and the classic page's wording reads as a specific legacy affordance for group claims rather than the whole profile being flattened.
- **A projection that addresses nothing is an error, not a wall of nulls.** `Projection` caught every per-element failure and substituted `null`, so a typo'd field returned one null per group — on a large tenant, hundreds of them, reading exactly like a working expression. Per-element tolerance is still there for genuinely heterogeneous collections, but when a projection resolves nowhere on *any* element it now reports why and lists what is readable: `collection projection returned nothing for any of the 250 elements — Unknown variable: 'nmae'. Readable on each element: id, name, objectClass, profile, type`. Covers both shapes of the mistake — an unbound identifier, which throws, and a sub-field absent everywhere (`.![profile.bogus]`), which reads as null because OEL property access is null-tolerant. Present-but-null is deliberately left alone: a group with no description really does project null, and erroring on that would break correct expressions.
...
- `simplePathSegments` / `pathAbsentOn` back the projection diagnostic. The first refuses anything that isn't a plain attribute path, so a compound projection that legitimately evaluates to null everywhere is never flagged; the second distinguishes absent from present-and-null, which is what keeps the diagnostic off correct expressions.
- Verification: 113 templates + 82 reference examples against the shipped profile and all six presets (1365 evaluations, zero failures), plus ~70 autocomplete and projection assertions driving `computeAutocomplete` / `parseCriteriaContext` / `parseProjectionContext` / `acceptAutocomplete` directly, plus every verbatim `getGroups`/`isMemberOf` example from **both** references — the classic page and the Identity Engine page document different projection spellings, and the audit asserts both.
