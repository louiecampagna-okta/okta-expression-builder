  ...
  const FUNCTION_REFERENCE = [
    ...
    },
    {
      ...
      fns: [
        ...
        { sig:"user.getGroups(criteria[, ...])", desc:"Returns the user's matching groups as group objects, so projections work. Criteria keys: group.id, group.type, group.source.id, group.profile.name. A list value matches any of its entries (OR); extra criteria objects must all match (AND). A projection can read any group attribute — id, type, created, lastUpdated, lastMembershipUpdated, profile.name and profile.description are the ones Okta illustrates.", ex:"user.getGroups({'group.type': {'OKTA_GROUP'}}).![profile.name]" },
